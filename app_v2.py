...
def _ensure_uploaded() -> None:
    if CACHE.raw_df is None or CACHE.stats is None:
        raise HTTPException(status_code=400, detail="No dataset uploaded or processed yet.")
...
def _normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Validate expected columns presence (case-insensitive)
    missing = [c for c in EXPECTED_COLUMNS if c.lower() not in df.columns]
    if missing:
        # Do not block if only optional columns are missing; here we require full set per spec
    ...
    for faers_col, canon in FAERS_COLUMN_MAP.items():
        if faers_col in df.columns:
            df[canon] = df[faers_col]
...
def _compute_prr_ror_ic(cont: pd.DataFrame) -> pd.DataFrame:
    ...
    # PRR CI on log scale
    se_log_prr = np.sqrt(
        (1.0 / (df["a"] + eps)) - (1.0 / (df["a"] + df["b"] + eps))
        + (1.0 / (df["c"] + eps)) - (1.0 / (df["c"] + df["d"] + eps))
    )
    df["PRR_LCL"] = np.exp(np.log(df["PRR"] + eps) - 1.96 * se_log_prr)

    # ROR CI on log scale
    se_log_ror = np.sqrt(
        (1.0 / (df["a"] + eps)) + (1.0 / (df["b"] + eps))
        + (1.0 / (df["c"] + eps)) + (1.0 / (df["d"] + eps))
    )
    df["ROR_LCL"] = np.exp(np.log(df["ROR"] + eps) - 1.96 * se_log_ror)

    # Flags
    df["meets_signal"] = ((df["PRR"] >= 2.0) & (df["a"] >= 3) & (df["PRR_LCL"] > 1.0)).astype(int)
...
def _prepare_cache(df: pd.DataFrame) -> None:
    ...
    # add severity
    df["severity"] = _severity_score(df.get("outcome", pd.Series(index=df.index, dtype=str)))
    ...
    sev = (
        df.groupby([CANON_DRUG, CANON_EVENT])["severity"].mean().reset_index(name="mean_severity")
    )
    ...
    try:
        ...
        ai_stats = ai_predictions.groupby([CANON_DRUG, CANON_EVENT]).agg({
            'is_signal': 'max',  # True if any record is flagged as signal
            'probability': 'mean',  # Average probability
            'PRR': 'first',
            'ROR': 'first',
            'is_disproportional_signal': 'max'
        }).reset_index()
        
        # Merge AI predictions with statistical results
        stats = stats.merge(ai_stats, on=[CANON_DRUG, CANON_EVENT], how="left", suffixes=('', '_ai'))
        
        # Update signal detection to include AI predictions
        stats['meets_signal'] = (
            (stats['meets_signal'] == 1) | 
            (stats.get('is_signal', 0) == 1) |
            (stats.get('is_disproportional_signal', 0) == 1)
        ).astype(int)
...
def _jsonify_df(df: pd.DataFrame, orient: str = "records") -> List[Dict[str, Any]]:
    ...
    for col in converted.columns:
        if np.issubdtype(converted[col].dtype, np.datetime64):
            converted[col] = converted[col].astype("datetime64[ns]").dt.strftime("%Y-%m-%d")
...
def critical_signals() -> Dict[str, Any]:
    ...
    stats = CACHE.stats
    sev = (
        df.groupby([CANON_DRUG, CANON_EVENT])["severity"].max().reset_index(name="max_severity")
    )
...
def top_drugs_signals(limit: int = 10) -> Dict[str, Any]:
    ...
    top = df.groupby(CANON_DRUG).size().reset_index(name="signal_count").sort_values(
...
def top_adverse_events(limit: int = 10) -> Dict[str, Any]:
    _ensure_uploaded()
    df = CACHE.stats[CACHE.stats["meets_signal"] == 1]
    top = df.groupby(CANON_EVENT).size().reset_index(name="signal_count").sort_values(
        "signal_count", ascending=False
    ).head(limit)
...
def quick_actions() -> Dict[str, Any]:
    ...
    actions: List[str] = []
    if high_risk > 0:
        actions.append("Initiate immediate safety review for high-risk pairs (PRR/ROR ≥ 5).")
    if signals > 50:
        actions.append("Prioritize top 20 signal pairs for medical assessment.")
    if total > 10000 and signals / max(total, 1) > 0.002:
        actions.append("Increase surveillance frequency; elevated signal rate for large cohort.")
    if not actions:
...
def system_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "signal_pairs": int(CACHE.stats["meets_signal"].sum()) if CACHE.stats is not None else 0,
        "ai_signal_pairs": int(CACHE.ai_predictions["is_signal"].sum()) if CACHE.ai_predictions is not None else 0,
    }
...
def risk_assessment(limit: int = 50) -> Dict[str, Any]:
    ...
    ranked = df.sort_values(["meets_signal", "risk_score"], ascending=[False, False]).head(limit)
    cols = [CANON_DRUG, CANON_EVENT, "a", "PRR", "ROR", "IC", "PRR_LCL", "ROR_LCL", "mean_severity", "meets_signal", "risk_score"]
...
def statistical_measures(limit: int = 100) -> Dict[str, Any]:
    _ensure_uploaded()
    cols = [CANON_DRUG, CANON_EVENT, "a", "b", "c", "d", "PRR", "ROR", "IC", "PRR_LCL", "ROR_LCL", "meets_signal"]
...
def drug_event_signal_pairs(limit: int = 100) -> Dict[str, Any]:
    ...
    df = CACHE.stats[CACHE.stats["meets_signal"] == 1][cols].sort_values("PRR", ascending=False).head(limit)
...
def ai_signals(limit: int = 100) -> Dict[str, Any]:
    ...
    ai_signals_df = CACHE.ai_predictions[CACHE.ai_predictions["is_signal"] == 1]
    cols = [CANON_DRUG, CANON_EVENT, "probability", "PRR", "ROR", "is_disproportional_signal"]
    result = ai_signals_df[cols].sort_values("probability", ascending=False).head(limit)
...
def ai_signal_count() -> Dict[str, Any]:
    ...
    _ensure_uploaded()
    if CACHE.ai_predictions is None:
        return {"ai_signal_count": 0, "message": "AI model not available"}
...
def ai_high_confidence_signals(threshold: float = 0.8, limit: int = 50) -> Dict[str, Any]:
    ...
    cols = [CANON_DRUG, CANON_EVENT, "probability", "PRR", "ROR"]
...
def ai_model_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "total_ai_predictions": len(CACHE.ai_predictions) if CACHE.ai_predictions is not None else 0,
    }
...
def hybrid_signals(limit: int = 100) -> Dict[str, Any]:
    ...
    cols = [CANON_DRUG, CANON_EVENT, "a", "PRR", "ROR", "IC", "mean_severity", "meets_signal"]
...
async def predict_single(
    ...
    if CACHE.ai_detector is None:
        raise HTTPException(status_code=400, detail="AI model not available. Upload dataset first.")
    ...
    try:
        ...
        return {
            "drug_name": drug_name,
            "adverse_event": adverse_event,
            "is_signal": bool(prediction["is_signal"]),
            "probability": float(prediction["probability"]),
            "PRR": float(prediction.get("PRR", 1.0)),
            "ROR": float(prediction.get("ROR", 1.0)),
            "is_disproportional_signal": bool(prediction.get("is_disproportional_signal", False))
        }
    except Exception as e:
...
def distribution_analysis() -> Dict[str, Any]:
    ...
    return {
        "age_histogram": {"bins": list(map(int, edges[:-1])), "counts": list(map(int, hist))},
...
def risk_heatmaps(limit_drugs: int = 20, limit_events: int = 20) -> Dict[str, Any]:
    ...
    top_drugs = (
        stats.groupby(CANON_DRUG)["meets_signal"].sum().sort_values(ascending=False).head(limit_drugs).index
    )
    top_events = (
        stats.groupby(CANON_EVENT)["meets_signal"].sum().sort_values(ascending=False).head(limit_events).index
    )
    sub = stats[stats[CANON_DRUG].isin(top_drugs) & stats[CANON_EVENT].isin(top_events)]
    pivot = sub.pivot_table(index=CANON_DRUG, columns=CANON_EVENT, values="PRR", fill_value=0)
    data = {
...
def top_drugs_detected(limit: int = 10) -> Dict[str, Any]:
    _ensure_uploaded()
    df = CACHE.stats[CACHE.stats["meets_signal"] == 1]
    top = df.groupby(CANON_DRUG).size().reset_index(name="signals").sort_values("signals", ascending=False).head(limit)
...
def most_frequent_adverse_events(limit: int = 10) -> Dict[str, Any]:
    _ensure_uploaded()
    top = CACHE.raw_df.groupby(CANON_EVENT).size().reset_index(name="count").sort_values("count", ascending=False).head(limit)
...
def signal_trends() -> Dict[str, Any]:
    ...
    pairs = set(zip(CACHE.stats.loc[CACHE.stats["meets_signal"] == 1, CANON_DRUG], CACHE.stats.loc[CACHE.stats["meets_signal"] == 1, CANON_EVENT]))
    df["is_signal_record"] = df[[CANON_DRUG, CANON_EVENT]].apply(lambda r: (r[CANON_DRUG], r[CANON_EVENT]) in pairs, axis=1)
    trend = df.groupby("fda_month")["is_signal_record"].mean().reset_index(name="signal_rate")
    return {"signal_trends": _jsonify_df(trend.rename(columns={"fda_month": "month"}))}


@app.get("/drug-event-severity-heatmap")
def drug_event_severity_heatmap(limit_drugs: int = 20, limit_events: int = 20) -> Dict[str, Any]:
    _ensure_uploaded()
    stats = CACHE.stats
    top_drugs = (
        stats.groupby(CANON_DRUG)["meets_signal"].sum().sort_values(ascending=False).head(limit_drugs).index
    ...
    sub = stats[stats[CANON_DRUG].isin(top_drugs) & stats[CANON_EVENT].isin(top_events)]
    pivot = sub.pivot_table(index=CANON_DRUG, columns=CANON_EVENT, values="mean_severity", fill_value=0)
    data = {
        "rows": list(pivot.index),
        "cols": list(pivot.columns),
        "values": [[float(x) for x in row] for row in pivot.values],
...
def key_insights() -> Dict[str, Any]:
    ...
    signals = int(CACHE.stats["meets_signal"].sum())
    high_risk = int(((CACHE.stats["PRR"] >= 5) | (CACHE.stats["ROR"] >= 5)).sum())
    avg_sev = float(CACHE.stats.loc[CACHE.stats["meets_signal"] == 1, "mean_severity"].mean() or 0.0)
    return {
        "insights": {
            "total_reports": total,
            "total_pairs": pairs,
...
def signal_distribution() -> Dict[str, Any]:
    _ensure_uploaded()
    stats = CACHE.stats
    by_drug = stats.groupby(CANON_DRUG)["meets_signal"].sum().reset_index(name="signals").sort_values("signals", ascending=False)
    by_event = stats.groupby(CANON_EVENT)["meets_signal"].sum().reset_index(name="signals").sort_values("signals", ascending=False)
    return {
